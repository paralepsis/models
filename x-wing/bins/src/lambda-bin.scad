...
module lambdaOutline() {
   translate([0,0,46.5/2]) rotate([180,0,0]) {
      blankCutout(height=46.5/2 - 0.1, outlineScale=1.2) lambdaBlank();
   }
}
...
module bottomAngleCutout() {
   ...
   rotate([0,0,90]) translate([0,0,0]) rotate([90,0,0]) linear_extrude(height=width)
